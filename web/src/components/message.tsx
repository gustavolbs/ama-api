...
import { markMessageAsSolved } from "../http/mark-as-solved";
...
export function Message({
  ...
  async function markAsSolved() {
    ...
    try {
      await markMessageAsSolved({ messageId, roomId });
    } catch {
      toast.error("Falha ao marcar como resolvido, tente novamente!");
    }
    ...
    >
      ...
      <div className="flex justify-between items-center">
        ...
        <button
          ...
          className="mt-3 flex items-center gap-2 text-zinc-400 text-sm font-medium hover:text-zinc-300"
        >
          Marcar como respondido <Check className="size-4" />
        </button>
      </div>
    </li>
  );
